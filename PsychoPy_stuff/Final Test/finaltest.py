...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\cheng\\OneDrive\\Desktop\\NeuroLingo\\Final Test (1)\\Final Test\\finaltest.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # create an object to store info about Routine COUNTERBALANCE
    COUNTERBALANCE = data.Routine(
        name='COUNTERBALANCE',
        components=[],
    )
    COUNTERBALANCE.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # store start times for COUNTERBALANCE
    COUNTERBALANCE.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    COUNTERBALANCE.tStart = globalClock.getTime(format='float')
    COUNTERBALANCE.status = STARTED
    thisExp.addData('COUNTERBALANCE.started', COUNTERBALANCE.tStart)
    COUNTERBALANCE.maxDuration = None
    # keep track of which components have finished
    COUNTERBALANCEComponents = COUNTERBALANCE.components
    for thisComponent in COUNTERBALANCE.components:
    ...
    COUNTERBALANCE.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        ...
        if not continueRoutine:  # a component has requested a forced-end of Routine
            COUNTERBALANCE.forceEnded = routineForceEnded = True
            break
    ...
    # store stop times for COUNTERBALANCE
    COUNTERBALANCE.tStop = globalClock.getTime(format='float')
    COUNTERBALANCE.tStopRefresh = tThisFlipGlobal
    thisExp.addData('COUNTERBALANCE.stopped', COUNTERBALANCE.tStop)
    thisExp.nextEntry()
    ...
    for thisPractice_images in practice_imagess:
        ...
        for thisPractice_sets2 in practice_sets2:
            ...
            continueRoutine = True
            # update component parameters for each repeat
            amharic_practice2.setText(practice_list2[practice_keys2[practice_index]][1])
    ...
    # completed 6.0 repeats of 'practice_imagess'
    ...
    start_test = data.Routine(
        name='start_test',
        components=[textWelcome, debugging_press_enter_to_skip_7],
    )
    ...
    while continueRoutine and routineTimer.getTime() < 1.0:
        ...
        if textWelcome.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > textWelcome.tStartRefresh + 1-frameTolerance:
                # keep track of stop time/frame for later
                textWelcome.tStop = t  # not accounting for scr refresh
                textWelcome.tStopRefresh = tThisFlipGlobal  # on global time
    ...
    thisExp.addData('debugging_press_enter_to_skip_7.keys',debugging_press_enter_to_skip_7.keys)
    if debugging_press_enter_to_skip_7.keys != None:  # we had a response
        thisExp.addData('debugging_press_enter_to_skip_7.rt', debugging_press_enter_to_skip_7.rt)
        thisExp.addData('debugging_press_enter_to_skip_7.duration', debugging_press_enter_to_skip_7.duration)
    # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
    if start_test.maxDurationReached:
        routineTimer.addTime(-start_test.maxDuration)
    elif start_test.forceEnded:
        routineTimer.reset()
    else:
        routineTimer.addTime(-1.000000)
    thisExp.nextEntry()
    ...
    while continueRoutine:
        ...
        if goNext1.status == STARTED and not waitOnFlip:
            theseKeys = goNext1.getKeys(keyList=['return'], ignoreKeys=["escape"], waitRelease=False)
