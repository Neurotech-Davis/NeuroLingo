...
plots = [] #delete later, just for real time plots 
curves = []#

for i in range(CHANNEL_COUNT): #for graphing 
...
def handle_sample(sample): #checking for new data 
    for i in range(CHANNEL_COUNT):
        value = sample.channels_data[i] * SCALE_FACTOR #converts to microvolts?
        eeg_buffers[i].append(value) #appending most recent values 

# --- Start Cyton streaming thread ---
def start_stream(): #initializes cyton board 
    print("Starting Cyton stream...")
    board = OpenBCICyton(port='COM8', daisy=False)
    board.start_stream(handle_sample) #starts streaming data 

stream_thread = threading.Thread(target=start_stream)
stream_thread.daemon = True #to not block main GUI
stream_thread.start()
